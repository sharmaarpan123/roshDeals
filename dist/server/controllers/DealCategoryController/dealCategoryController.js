import { addDealCategorySchema, deleteDealCategorySchema, editDealCategorySchema, } from './schema.js';
...
const editDealCategoryController = catchAsync(async (req, res) => {
    ...
    const UpdatedDealCategoryForm = await DealCategory.findByIdAndUpdate({ _id: dealCategoryId }, { name: name }, { new: true });
...
const deleteDealCategoryController = catchAsync(async (req, res) => {
    ...
    const UpdatedDealCategoryForm = await DealCategory.findByIdAndUpdate({ _id: dealCategoryId }, {
        isDeleted: true,
    }, { new: true });
    if (UpdatedDealCategoryForm) {
        return res.status(200).json(successResponse({
