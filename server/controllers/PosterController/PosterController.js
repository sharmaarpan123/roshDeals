import { addSchema, deleteSchema, editSchema, statusChangeSchema, } from './schema.js';
...
const editPosterController = catchAsync(async (req, res) => {
    ...
    const updatedPoster = await Poster.findByIdAndUpdate({ _id: posterId }, {
        name,
        title,
        image,
...
const deletePosterController = catchAsync(async (req, res) => {
    ...
    const { posterId } = body;
    const deletedData = await Poster.findByIdAndUpdate({ _id: posterId }, {
        isDeleted: true,
    }, { new: true });
    if (deletedData) {
        return res.status(200).json(successResponse({
...
const statusChangeController = catchAsync(async (req, res) => {
    ...
    const data = await Poster.findByIdAndUpdate({ _id: posterId }, {
        isActive: status,
    }, { new: true });
    if (data) {
        return res.status(200).json(successResponse({
